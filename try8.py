...
for cat in categories:
	predAnswers = defaultdict(lambda: defaultdict(float))
	predBigramAnswers = defaultdict(lambda: defaultdict(float))
	...
	if cat=='science':
		...
		pred = DictReader(open("predScienceBigrams3.csv", 'r'))
		for ii in pred:
			answers=ast.literal_eval(ii['Answer'])
			qid=ast.literal_eval(ii['Question ID'])
			for kk in answers:
				predBigramAnswers[str(qid)][str(kk[0])]=float(kk[1])
	...
	if cat=='history':
		pred = DictReader(open("predHistoryText3.csv", 'r'))
		for ii in pred:
			answers=ast.literal_eval(ii['Answer'])
			qid=ast.literal_eval(ii['Question ID'])
			for kk in answers:
	...
	for ii in testFile:
		if ii['category']==cat:
			...
			for answer in answerSet:
				...
				if (ii['category']=='science') or (ii['category']=='social') or (ii['category']=='history') or (ii['category']=='lit'):
				...
				if (ii['category']=='science') or (ii['category']=='social') or (ii['category']=='history') or (ii['category']=='lit'):
					if predAnswers[ii['Question ID']][answer]>0:
						# print "TextScore=",predAnswers[int(ii[0])][answer]
						biScore=predBigramAnswers[ii['Question Text']][answer]
					else:
						biScore=0.0
				# else:
				# 	#Get bigrams score
...
# Write predictions
o = DictWriter(open('pred8.csv', 'wb'), ['Question ID', 'Answer'])
o.writeheader()
